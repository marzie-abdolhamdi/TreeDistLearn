...
# 4 (DNA_uv) + 1 (scale_uv) + 4 (DNA_vu) + 1 (scale_vu) + 4 (DNA_new_leaf)
EDGE_FEATURE_DIM = 14
...
def edge_feature(
    cav_uv, scale_uv, cav_vu, scale_vu, new_leaf_lik, n_nodes: int
) -> torch.Tensor:
    ...
    scale_uv_n   = (scale_uv / n_nodes).unsqueeze(-1)
    scale_vu_n   = (scale_vu / n_nodes).unsqueeze(-1)
    return torch.cat([cav_uv_n, scale_uv_n, cav_vu_n, scale_vu_n, y_n], dim=-1)


def build_edge_features(
    edges, cav_cache, new_leaf_lik, device, n_nodes: int
) -> torch.Tensor:
    new_leaf_lik = new_leaf_lik.to(device)
    feats = []
    for u, v in edges:
        cav_uv, scale_uv = cav_cache[(u, v)]
        cav_vu, scale_vu = cav_cache[(v, u)]
        feats.append(edge_feature(
            cav_uv.to(device), scale_uv.to(device),
            cav_vu.to(device), scale_vu.to(device),
            new_leaf_lik, n_nodes=n_nodes,
        ))
    return torch.stack(feats, dim=0)   # (E, L, 14)
...
class RhoNet(nn.Module):
    ...
    def get_pooled(self, x: torch.Tensor) -> torch.Tensor:
        """
        Masked average pooling over informative (non-gap) sites.
        x : (E, L, C)  →  h : (E, hidden)
        """
        xt   = x.transpose(1, 2)                       # (E, C, L)
        y_n  = xt[:, 10:14, :]                         # new-leaf channels
        mask = (y_n.max(dim=1)[0] > 0.26).float()      # (E, L)
        h_seq = self.cnn(xt)                            # (E, hidden, L)
        mask  = mask.unsqueeze(1)                       # (E, 1, L)
        valid = mask.sum(dim=2).clamp_min(1.0)          # (E, 1)
...
def compute_loglik_all_edges(
    ...
    m_u = torch.einsum("elc,kecd->keld", psi_u, P_uw)              # (K,E,L,4)
    m_v = torch.einsum("elc,kecd->keld", psi_v, P_wv)
    m_y = torch.einsum("lc,kecd->keld",  new_leaf_lik, P_yw)
...
def forward_ca(
    ...
    z_b = (mu_b + std_b * eps_b).clamp(max=15.0)
    b   = torch.exp(z_b)                                  # (K,E)
    ...
    # ── Per-edge ELBO ─────────────────────────────────────────────────────────
    n_sites = new_leaf_lik.shape[0]  # Grab the sequence length
    if phase == 1:
        elbo_e = loglik_mc - n_sites * kl_rho - n_sites * kl_b_mc
    else:
        # kl_rho is constant wrt BNet → detach it to get a clean BNet gradient,
        # but include it so the logsumexp edge weighting remains theoretically correct.
        elbo_e = loglik_mc - n_sites * kl_rho.detach() - n_sites * kl_b_mc
...
def evaluate_edges_ca(
    ...
    std_b  = torch.exp(log_sigma_b)
    eps_b  = torch.randn(k_samples, n_edges, device=device)
    b = torch.exp((mu_b + std_b * eps_b).clamp(max=15.0))               # (K,E)
...
def choose_action_ca(
    ...
    For the chosen edge, rho is sampled first; b is then drawn from
    BNet conditioned on that specific rho.
    """
    ...
    if sample_continuous:
        # Sample rho
        sigma_r    = torch.exp(log_sigma_r[chosen_idx]).item()
        z_r        = random.gauss(mu_r[chosen_idx].item(), sigma_r)
        chosen_rho = 1.0 / (1.0 + math.exp(-z_r))   # sigmoid

        # Query BNet conditioned on chosen_rho
        h_e       = h[chosen_idx].detach().unsqueeze(0)                 # (1, hidden)
        rho_in    = torch.tensor([[chosen_rho]], dtype=torch.float32,
                                  device=device)                          # (1, 1) → (K=1, E=1)
        with torch.no_grad():
            mu_b_i, lsb_i = b_net(h_e, rho_in)
        mu_b_i  = mu_b_i.item()
        lsb_i   = lsb_i.item()

        # Sample b
        z_b      = random.gauss(mu_b_i, math.exp(lsb_i))
        chosen_b = math.exp(min(z_b, 15.0))
    else:
        # Deterministic mode (MAP)
        ...
        chosen_b   = math.exp(min(mu_b_i.item(), 15.0))
...
def save_step_plots(
    ...
    plt.xlabel("Edge"); plt.ylabel("q(e) = softmax(ELBO*)")
    plt.title("Edge Posterior (CA-VI ELBO*)"); plt.tight_layout()
    plt.savefig(os.path.join(out_dir, "edge_posterior_qe.png"), dpi=150)
    plt.close()

    # ── Relative ELBO per edge ────────────────────────────────────────────────
    rel_elbo = elbo_final - np.max(elbo_final)
    plt.figure(figsize=(10, 5))
    plt.bar(edge_labels, rel_elbo)
    plt.xticks(rotation=45, ha="right", fontsize=8)
    plt.xlabel("Edge"); plt.ylabel("ELBO* – max(ELBO*)")
    plt.title("Relative Per-Edge ELBO Scores"); plt.tight_layout()
    ...
    b_means = np.exp(mu_b_final_mean)
    b_stds  = b_means * np.exp(log_sigma_b_final_mean)
...
def train_single_insertion_step(
    ...
    for step_iter in range(1, args.max_steps + 1):
        ...
        with torch.no_grad():
            for step in range(3, taxon_idx):
                ...
                (edges_s, q_e_s, _, mu_r_s, lsr_s, _, _, h_s,
                 _, _, _, _, _, _, _) = evaluate_edges_ca(
                ...
                chosen_edge, chosen_rho, chosen_b = choose_action_ca(
                    edges_s, q_e_s, mu_r_s, lsr_s, h_s, b_net_s, device,
        ...
        q_e_eval = torch.softmax(elbo_e_eval / current_temp, dim=0)
    ...
    if step_out_dir and final_tree is not None:
        ...
        with torch.no_grad():
            (edges_f, q_e_f, elbo_e_f,
             ...
             rho_np_f, b_np_f,
             _, _, _, _, edge_feats_f) = evaluate_edges_ca(
...
def sample_tree_with_log_q(
    ...
    with torch.no_grad():
        for taxon_idx in range(3, n_use):
            ...
            # ── Compute per-edge ELBO for edge selection (temperature=1) ──────
            # FIX: Use the mode of the latent Gaussians instead of MC samples
            # to compute a fixed, deterministic proposal distribution q(e).
            z_r_det = mu_r.unsqueeze(0)                           # (1, E)
            rho_det = torch.sigmoid(z_r_det)                      # (1, E)

            # Pass deterministic rho to BNet
            mu_b_det, log_sigma_b_det = b_net(h.detach(), rho_det) # (1, E)
            b_det = torch.exp(mu_b_det)                           # (1, E)

            # Compute KL terms deterministically
            kl_rho = gaussian_kl_standard(mu_r, log_sigma_r)      # (E,)
            kl_b   = exponential_kl_branch(mu_b_det, log_sigma_b_det).squeeze(0) # (E,)
            
            # Compute likelihood deterministically
            _sc = _stack_cavities(edges, tree, cav_cache, device)
            loglik_det = compute_loglik_all_edges(
                *_sc, rho_det, b_det, new_lik, pi_dna, args.jc_rate
            ) # (E,)
            
            # Deterministic ELBO -> Stable proposal q(e)
            n_sites = new_lik.shape[0]
            elbo_e = loglik_det - n_sites * kl_rho - n_sites * kl_b
            q_e    = torch.softmax(elbo_e, dim=0)        # temp=1 for unbiased IS
            ...
            # ── Sample rho for chosen edge ────────────────────────────────────
            mu_r_i  = mu_r[chosen_idx].item()
            lsr_i   = log_sigma_r[chosen_idx].item()
            z_r_i   = random.gauss(mu_r_i, math.exp(lsr_i))
            chosen_rho = 1.0 / (1.0 + math.exp(-z_r_i))

            # ── Sample b conditioned on chosen_rho ────────────────────────────
            h_e     = h[chosen_idx].detach().unsqueeze(0)               # (1,H)
            rho_in  = torch.tensor([[chosen_rho]], dtype=torch.float32,
                                    device=device)                       # (1,1)
            mu_b_i, lsb_i = b_net(h_e, rho_in)
            mu_b_i  = mu_b_i.item()
            lsb_i   = lsb_i.item()
            z_b_i   = random.gauss(mu_b_i, math.exp(lsb_i))
            chosen_b = math.exp(z_b_i)
    ...
    # ── log p(b) for all 2N-3 edges under Exp(λ) ─────────────────────────────
    lam    = PRIOR_LAM_B
    log_p_b_all = sum(
    ...
    # THE FIX: Subtract the prior penalty of the 3 fixed initial branches 
    # so that log_p_b perfectly matches the branches sampled in log_q
    initial_prior = 3 * math.log(lam) - lam * (args.edge_len_0 + args.edge_len_1 + args.edge_len_2)
    log_p_b = log_p_b_all - initial_prior
...
def estimate_mll(
    ...
    for _ in tqdm(range(k_mll), desc="MLL samples", leave=False, dynamic_ncols=True):
        ...
        if log_p_Y is None:
            continue
        log_weights.append(log_p_Y + log_p_b - log_q)
...
def save_sampled_tree_plots(
    ...
    with torch.no_grad():
        for tidx in range(3, n_taxa):
            ...
            e_c, r_c, b_c = choose_action_ca(
                edges_t, q_t, mu_r_t, lsr_t, h_t, bn, device, sample_continuous=False
            )
    ...
    # ── Loglik sanity (5 edges, prior mean b) ────────────────────────────────
    pi_dna     = PI_DNA.to(device)
    edges_f    = tree_tmp.edges()
    site_lls   = []
    with torch.no_grad():
        for u, v in edges_f[:min(5, len(edges_f))]:
            ...
            ll    = insertion_loglik_from_edge_batched(
                ...
                leaf_lik_all[last_tidx].to(device),
                cav_final, pi_dna, rate=args.jc_rate,
            )
            site_lls.append(ll.item())

    plt.figure(figsize=(8, 4))
    plt.bar(range(len(site_lls)), site_lls, color='teal', alpha=0.8)
    plt.axhline(0, color='red', linestyle='--')
    plt.xlabel("Edge index (first 5)"); plt.ylabel("Total log-lik (sum over sites)")
    plt.title("Cat 5: Per-Edge Log-Likelihood Check"); plt.grid(alpha=0.3)
    plt.tight_layout()
