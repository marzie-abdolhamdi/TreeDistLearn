...
class RhoNet(nn.Module):
    ...
    def get_pooled(self, x: torch.Tensor) -> torch.Tensor:
        ...
        y_n  = xt[:, 8:12, :]                          # new-leaf channels (indices 8,9,10,11)
...
def choose_action_ca(
    ...
    if sample_continuous:
        ...
        chosen_rho = float(torch.sigmoid(torch.tensor(z_r)))  # Safe Sigmoid
        ...
        chosen_b = max(1e-8, math.exp(min(z_b, 15.0)))
    else:
...
def sample_tree_with_log_q(
    ...
    with torch.no_grad():
        for taxon_idx in range(3, n_use):
            ...
            # SCORE EDGES STRICTLY BY FELSENSTEIN LIKELIHOOD 
            q_e    = torch.softmax(loglik_det, dim=0)    # temp=1 for unbiased IS
            ...
            chosen_rho = float(torch.sigmoid(torch.tensor(z_r_i))) # Safe Sigmoid
            ...
            chosen_b = max(1e-8, math.exp(min(z_b_i, 15.0)))
