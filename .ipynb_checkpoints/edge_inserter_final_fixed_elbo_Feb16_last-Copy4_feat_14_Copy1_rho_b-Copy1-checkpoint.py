...
def forward_ca(
    ...
    loglik_mc = compute_loglik_per_edge(
        tree, edges, rho, b, new_leaf_lik, cav_cache, pi, jc_rate
    )  # (E,)
...
def evaluate_edges_ca(
    ...
    loglik_mc  = compute_loglik_per_edge(
        tree, edges, rho, b, new_leaf_lik, cav_cache, pi_dna, jc_rate
    )
...
def save_step_plots(
    ...
    plt.plot(steps, ema,       linewidth=2, label="ELBO (EMA α=0.05)")
...
def save_crosstep_plots(
    ...
    plt.figure(figsize=(max(8, len(tidxs) * 0.5), 4))
    ...
    plt.xticks(x, tlabels, rotation=45, ha='right', fontsize=8)
    plt.xlabel("Taxon"); plt.ylabel("Final ELBO / n_sites")
    plt.title("ELBO at Convergence per Taxon"); plt.grid(alpha=0.3); plt.tight_layout()
...
def sample_tree_with_log_q(
    ...
    with torch.no_grad():
        for taxon_idx in range(3, n_use):
            ...
            loglik_mc = compute_loglik_per_edge(
                tree, edges, rho_k, b_k, new_lik, cav_cache, pi_dna, args.jc_rate
            )
...
def save_sampled_tree_plots(
    ...
    for rank, (nwk, cnt) in enumerate(top_topos[:5]):
