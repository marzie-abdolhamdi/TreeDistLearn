...
import edge_inserter_final_fixed_elbo_Feb16_last_phylo_VI_Copy2 as vi
...
def main():
    ...
    vi_args = argparse.Namespace(
        ...
        k_eval=100, jc_rate=1.0, sample_temp=1.0, hidden_dim=args.hidden_dim
    )
    ...
    for k in range(4, len(names) + 1):
            ...
            with torch.no_grad():
                for taxon_idx in range(3, k):
                    ...
                    edges, q_e, _, mu_r, lsr, _, _, h, _, _, _, _, _, _, _ = vi.evaluate_edges_ca(
                        ...
                        rho_net, b_net, device, k_samples=vi_args.k_eval, jc_rate=1.0, temperature=1.0
                    )
        ...
        # --- Print the AI's Empirical Distribution with Tree Shapes ---
        if match_pct < 100.0:  
            print(f"\n         [Deep Dive at Step {k}: {names_k[-1]}]")
            print(f"         Out of {args.n_samples} samples, here is what the VI model built:")
            
            vi_topo_counts = {}
            vi_topo_reps = {}  # Save one example tree to show the user
            
            for vt in vi_trees:
                b_hash = frozenset(
                    b.split_bitmask if hasattr(b, 'split_bitmask') else b 
                    for b in vt.bipartition_encoding
                )
                vi_topo_counts[b_hash] = vi_topo_counts.get(b_hash, 0) + 1
                
                # Save the first tree we see of this shape
                if b_hash not in vi_topo_reps:
                    vi_topo_reps[b_hash] = vt
            
            sorted_vi_topos = sorted(vi_topo_counts.items(), key=lambda x: x[1], reverse=True)
            
            for rank, (b_hash, count) in enumerate(sorted_vi_topos[:3]):  # Shows top 3 most picked trees
                is_correct = b_hash in unique_oracle
                status = "✅ CORRECT (Matches MrBayes)" if is_correct else "❌ WRONG"
                
                # Get the human-readable shape (strips out all the branch length noise)
                rep_tree = vi_topo_reps[b_hash]
                shape_str = rep_tree.as_string(schema="newick", suppress_edge_lengths=True).strip()
                
                print(f"            Rank {rank+1}: Generated {count} times ({count/args.n_samples*100:.1f}%) -> {status}")
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(args.run_dir, "step_by_step_diagnosis_1.png"), dpi=150)
    print(f"\nDiagnosis complete! Plot saved to {args.run_dir}/step_by_step_diagnosis_1.png")
