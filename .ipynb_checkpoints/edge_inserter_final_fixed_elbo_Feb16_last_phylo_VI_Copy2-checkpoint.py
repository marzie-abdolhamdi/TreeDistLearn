...
def forward_ca(
    ...
    b   = torch.exp(z_b).clamp(min=1e-8, max=2.0)                                  # (K,E)
...
def evaluate_edges_ca(
    ...
    std_b  = torch.exp(log_sigma_b)
    eps_b  = torch.randn(k_samples, n_edges, device=device)
    b = torch.exp(mu_b + std_b * eps_b).clamp(min=1e-8, max=2.0)                    # (K,E)
...
def choose_action_ca(
    ...
    if sample_continuous:
        ...
        chosen_b = max(1e-8, math.exp(min(z_b, 2.0)))
    else:
...
def sample_tree_with_log_q(
    ...
    with torch.no_grad():
        for taxon_idx in range(3, n_use):
            ...
            b_det = torch.exp(mu_b_det).clamp(min=1e-8, max=2.0)                           # (1, E)
            ...
            chosen_b = max(1e-8, math.exp(min(z_b_i, 2.0)))
