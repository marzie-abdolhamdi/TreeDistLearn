...
class RhoNet(nn.Module):
    """
    Sequence-to-rho-params network with Learnable Soft Attention.
    
    Input : edge_feats  (E, L, EDGE_FEATURE_DIM)
    Output: mu_r        (E,)
            log_sigma_r (E,)
            h           (E, hidden)   ← attention-pooled cavity embedding, reused by BNet
    """
    def __init__(self, in_channels: int, hidden: int):
        super().__init__()
        self.cnn = nn.Sequential(
        ...
        # NEW: Soft Attention Head to score the phylogenetic importance of each site
    ...
    def get_pooled(self, x: torch.Tensor) -> torch.Tensor:
        """
        Soft Attention pooling over the sequence length, masking out 100% gap sites.
        x : (E, L, C)  →  h : (E, hidden)
        """
        xt = x.transpose(1, 2)                           # (E, C, L)
        
        # 1. Identify valid sites. A gap is 0.25, so max > 0.26 implies real data.
        # We look at anc_u, anc_v, AND the new leaf (y_n).
        u_max = xt[:, 0:4, :].max(dim=1)[0]              # (E, L)
        v_max = xt[:, 4:8, :].max(dim=1)[0]
        y_max = xt[:, 8:12, :].max(dim=1)[0]
        
        # Keep site if AT LEAST ONE node has data (Logical OR)
        site_mask = torch.clamp((u_max > 0.26).float() + 
                                (v_max > 0.26).float() + 
                                (y_max > 0.26).float(), max=1.0) # (E, L)
        
        # 2. Extract CNN features
        h_seq = self.cnn(xt)                             # (E, hidden, L)
        h_seq_trans = h_seq.transpose(1, 2)              # (E, L, hidden)
        
        # 3. Compute Attention Logits
        attn_logits = self.attention(h_seq_trans).squeeze(-1)    # (E, L)
        
        # 4. Apply Hard Mask to Logits
        # Sites with no data get -1e9, pushing their softmax probability to 0
        attn_logits = attn_logits + (1.0 - site_mask) * -1e9
        
        # 5. Softmax to get attention weights across sequence length L
        attn_weights = torch.softmax(attn_logits, dim=-1)        # (E, L)
        
        # 6. Weighted sum over the sequence length
        attn_weights = attn_weights.unsqueeze(-1)                # (E, L, 1)
        h = (h_seq_trans * attn_weights).sum(dim=1)              # (E, hidden)
...
def choose_action_ca(
    ...
    if sample_continuous:
        ...
        chosen_b = max(1e-8, min(math.exp(mu_b_i.item()), 2.0))
...
def sample_tree_with_log_q(
    ...
    with torch.no_grad():
        for taxon_idx in range(3, n_use):
            ...
            b_samp   = torch.exp(z_b_samp).clamp(min=1e-8, max=2.0)
            ...
            # SCORE EDGES STRICTLY BY FELSENSTEIN LIKELIHOOD 
            q_e = torch.softmax(loglik_samp, dim=0)    # temp=1 for unbiased IS
