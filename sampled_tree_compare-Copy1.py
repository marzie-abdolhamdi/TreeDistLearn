...
def _parse_translate_block(lines: List[str]) -> Dict[str, str]:
    ...
    for entry in block.split(","):
        ...
        if len(parts) >= 2:
            # Strip surrounding single or double quotes from the taxon name.
            # MrBayes often writes: 1 'Taxon1' — keeping the quotes as part
            # of the label causes frozenset key mismatches vs our unquoted names.
            name = parts[1].strip("'\"")  # remove surrounding single or double quotes
            tmap[parts[0]] = name
    return tmap
...
def strip_nhx_annotations(nwk: str) -> str:
    """
    Remove inline NHX/MrBayes [&...] annotations from a Newick string.
    MrBayes .trprobs trees contain per-node annotations like
        Taxon1[&prob=1.00,&prob_range=(1.00,1.00)]:0.1
    which dendropy (schema="newick") may interpret as part of the taxon label,
    creating a new Taxon "Taxon1[&prob=...]" that never matches "Taxon1" in
    the shared namespace — causing every bipartition key to be wrong.
    """
    return re.sub(r"\[&[^\]]*\]", "", nwk)


def build_shared_namespace(
    ours_newicks: List[str],
    mb_newicks: List[str],
) -> "dendropy.TaxonNamespace":
    """
    Build a shared TaxonNamespace from BOTH sources.

    Fixes vs original:
      1. Takes ours and MB separately so we always include at least one
         tree from each source (original [:20] slice could miss MB entirely
         when ours has 1000 trees).
      2. Strips [&...] NHX annotations before parsing so taxon labels
         are clean strings like "Taxon1", not "Taxon1[&prob=1.00,...]".
      3. After building, locks the namespace (is_mutable=False) to prevent
         dendropy from silently adding new taxa during bipartition computation,
         which would corrupt len(tns) and break the trivial-bipartition filter.
    """
    tns = dendropy.TaxonNamespace()
    # Sample up to 5 from each source to guarantee both are represented
    sample = ours_newicks[:5] + mb_newicks[:5]
    for nwk in sample:
        try:
            dendropy.Tree.get(
                data=strip_nhx_annotations(nwk) + ";",
                schema="newick",
                taxon_namespace=tns,
            ...
            pass
    tns.is_mutable = False   # Fix 2: lock namespace after building
    return tns
...
def newick_to_bipartition_key(
    ...
    """
    Convert a Newick string to a frozenset of integer split bitmasks.

    Uses integer bitmasks (not taxon label strings) for topology identity.
    This is the same approach as the attached split-frequency script and is
    immune to label formatting differences (quotes, whitespace, encoding)
    that cause frozenset-of-strings comparisons to silently fail.

    The bitmask is canonical (lowest bit always in leafset) so rooting and
    traversal order do not affect the result.
    ...
    try:
        t = dendropy.Tree.get(
            ...
            taxon_namespace=tns,
            preserve_underscores=True,
            rooting="force-unrooted",   # parse as unrooted from the start;
                                        # avoids degree-2 root that generates
                                        # extra/duplicate bipartitions when
                                        # is_rooted=False is set post-hoc
        )
        t.encode_bipartitions()
        n = len(tns)
        full_mask = (1 << n) - 1   # all n bits set
        bitmasks = []
        for edge in t.postorder_edge_iter():
            bp = edge.bipartition
            if bp is None:
                continue
            leafset = bp.leafset_bitmask
            n_set = bin(leafset).count("1")
            if 1 < n_set < n:
                # Normalise: a split and its complement represent the same
                # bipartition. Always store the lesser value so that trees
                # traversed from different roots/directions produce the same key.
                complement = (~leafset) & full_mask
                bitmasks.append(min(leafset, complement))
        return frozenset(bitmasks)
    except Exception:
        return None
...
def plot_rf_distances(
    ...
    def rf_between(a_trees, b_trees, max_pairs=2000, exclude_self=False):
        # exclude_self=True when a_trees is b_trees (within-set comparison)
        # to avoid RF(tree_i, tree_i)=0 pairs inflating the RF=0 bin.
        dists = []
        pairs = [
            (i, j)
            for i in range(len(a_trees))
            for j in range(len(b_trees))
    ...
    rf_within_ours = rf_between(ours_trees, ours_trees, exclude_self=True)
    rf_within_mb   = rf_between(mb_trees,   mb_trees,   exclude_self=True)
