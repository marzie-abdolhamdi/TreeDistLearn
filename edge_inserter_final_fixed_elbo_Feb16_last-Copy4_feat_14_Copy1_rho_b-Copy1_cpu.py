...
def forward_ca(
    ...
    if phase == 1:
        ...
        # Pass detached rho and h to BNet – no gradient leak into BNet
    ...
    if phase == 1:
        ...
        # kl_rho is constant wrt BNet → exclude to get clean gradient
        elbo_e = loglik_mc - kl_b_mc
...
def evaluate_edges_ca(
    ...
    std_b  = torch.exp(log_sigma_b)
    eps_b  = torch.randn(k_samples, n_edges, device=device)
    b      = torch.exp(mu_b + std_b * eps_b)               # (K,E)
...
def save_step_plots(
    ...
    axes[0].set_title("E[log p(data)]"); axes[0].set_ylabel("Nats"); axes[0].grid(alpha=0.3)
    axes[1].plot(steps, kl_b_hist, color='orange', label="KL_b (branch)")
    axes[1].plot(steps, kl_r_hist, color='red',    label="KL_r (rho)")
    axes[1].set_title("KL Terms"); axes[1].legend(); axes[1].grid(alpha=0.3)
...
def to_newick_topology_only(
    ...
    """
    Canonical topology string with NO branch lengths, for topology comparison.
    Children at each node are sorted lexicographically so the same topology
    always produces the same string regardless of traversal order.
    """
    start_node = next(
        (u for u in tree.adj if u >= len(taxon_names)), None
    ) or next(iter(tree.adj), None)
    ...
    def build_sub(u: int) -> str:
        ...
        children = [v for v in tree.neighbors(u) if v not in visited]
        if not children:
            return taxon_names[u]           # leaf: just name, no length
        subtrees = sorted(build_sub(v) for v in children)   # canonical order
        return "(" + ",".join(subtrees) + ")"

    visited.add(start_node)
    roots = sorted(build_sub(v) for v in tree.neighbors(start_node))
    return "(" + ",".join(roots) + ");"
...
def sample_tree_with_log_q(
    ...
    with torch.no_grad():
        for taxon_idx in range(3, n_use):
            ...
            std_r  = torch.exp(log_sigma_r)
            eps_r  = torch.randn(args.k_eval, n_edges, device=device)
            z_r    = mu_r.unsqueeze(0) + std_r.unsqueeze(0) * eps_r   # (K,E)
            rho_k  = torch.sigmoid(z_r)                                # (K,E)

            mu_b_k, log_sigma_b_k = b_net(h.detach(), rho_k)          # (K,E)
            std_b  = torch.exp(log_sigma_b_k)
            eps_b  = torch.randn(args.k_eval, n_edges, device=device)
            b_k    = torch.exp(mu_b_k + std_b * eps_b)                # (K,E)

            kl_rho = gaussian_kl_standard(mu_r, log_sigma_r)          # (E,)
            kl_b   = exponential_kl_branch(mu_b_k, log_sigma_b_k).mean(0)  # (E,)
            _sc = _stack_cavities(edges, tree, cav_cache, device)
            loglik_mc = compute_loglik_all_edges(
                *_sc, rho_k, b_k, new_lik, pi_dna, args.jc_rate
            )
            elbo_e = loglik_mc - kl_rho - kl_b
